...
(function () {
    ...
    function renderTroubleshootingTab(trob) {
        if (!trob || !trob.error_codes || !trob.error_codes.length) return '';
        var html = '<section class="py-6 md:py-8 px-4 md:px-6 doc-section-wrapper"><div class="max-w-4xl mx-auto">';
        html += '<div class="mb-6"><h2 class="text-xl md:text-2xl font-bold mb-2">' + (trob.title || 'Troubleshooting') + '</h2>';
        html += '<p class="text-sm text-gray-500">' + (trob.desc || '') + '</p></div>';
        html += '<div class="overflow-x-auto"><table class="config-table"><thead><tr><th>ErrCode</th><th>Section</th><th>Message</th><th>Resolution</th></tr></thead><tbody>';
        trob.error_codes.forEach(function (row) {
            html += '<tr><td><code>' + row.code + '</code></td><td>' + row.section + '</td><td>' + inlineCode(row.message) + '</td><td>' + row.resolution + '</td></tr>';
        });
    ...
    function renderDocsPage(data) {
        ...
        // Troubleshooting (genie doctor error codes)
        html += '<div id="section-troubleshooting" class="docs-section">' + renderTroubleshootingTab(data.troubleshooting) + '</div>';
