...
// NewManager creates a Manager from the given Config. When secrets are
// configured, a background goroutine eagerly resolves all runtimevar URLs
// and keeps them fresh via ttlcache.Item.KeepItFresh. If Config.Secrets is
...
func NewManager(ctx context.Context, cfg Config) *Manager {
	...
	if len(cfg.Secrets) > 0 {
		...
		manager.cancelBg = cancel
		go func() {
			manager.rawCache.GetValue(ctx)      //nolint:errcheck
			manager.rawCache.KeepItFresh(bgCtx) //nolint:errcheck
		}()
	}
...
func (m *Manager) resolveAllSecrets(ctx context.Context) (remoteSecretMap, error) {
	...
	logr.Debug("Resolving all remote secrets")
	defer func(startTime time.Time) {
		logr.Debug("time taken to resolve all remote secrets", "duration", time.Since(startTime).String())
	}(time.Now())
...
func (m *Manager) GetSecret(ctx context.Context, name string) (string, error) {
	...
	logr.Debug("Parsing secret name", "baseName", baseName, "nameJSONPath", nameJSONPath)

	varURL, hasURL := m.secretURLs[baseName]
	if !hasURL {
		logr.Debug("No runtimevar URL mapping found, falling back to env var")
		rawValue := os.Getenv(baseName)
		if nameJSONPath == "" {
			return rawValue, nil
		}
		return extractJSONPath(baseName, rawValue, nameJSONPath)
	}

	resolved, err := m.rawCache.GetValue(ctx)
	if err != nil {
		return "", fmt.Errorf("failed to resolve secrets from runtimevar: %w", err)
	}
	...
	return extractJSONPath(baseName, rawValue, jsonPath)
}
...
func (m *Manager) resolveFromRuntimevar(ctx context.Context, name, varURL string) (string, error) {
	logr := logger.GetLogger(ctx).With("fn", "Manager.resolveFromRuntimevar", "name", name)
	logr.Debug("Resolving secret from runtimevar")
	defer func(startTime time.Time) {
		logr.Debug("time taken to resolve secret from runtimevar", "duration", time.Since(startTime).String())
	}(time.Now())
