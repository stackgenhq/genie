...
// Scopes for Calendar, Contacts, Gmail, Drive, Tasks, Custom Search, and user profile (one sign-in powers all).
...
func RunBrowserFlow(ctx context.Context, credsJSON []byte) error {
	...
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if err := security.KeyringSet(keyringService, keyringAccount, tokenJSON); err != nil {
			http.Error(w, "failed to save token", http.StatusInternalServerError)
		...
		if name != "" || email != "" {
			...
			_ = security.KeyringSet(keyringService, keyringAccountUser, userJSON)
		}
...
<p><a href="https://appcd-dev.github.io/stackgen-genie/chat.html" style="color: #06c; font-weight: 600;">Open Genie Chat</a> to start chatting with Genie.</p>
