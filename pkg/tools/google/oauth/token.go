...
// Keyring identifiers for the Google OAuth token and user profile (stored via security.KeyringGet/Set).
const keyringService = "genie"
const keyringAccount = "google_oauth_token"
const keyringAccountUser = "google_oauth_user"
...
func GetToken(ctx context.Context, sp security.SecretProvider) (tokenJSON []byte, save func([]byte) error, err error) {
	...
	if tokenInline != "" {
		...
		save := func(b []byte) error {
			return security.KeyringSet(keyringService, keyringAccount, b)
		}
		return data, save, nil
	}

	data, err := security.KeyringGet(keyringService, keyringAccount)
...
func GetStoredUserInfo() (name, email string) {
	data, err := security.KeyringGet(keyringService, keyringAccountUser)
	if err != nil || len(data) == 0 {
		return "", ""
	}
	var v struct {
