...
func (s *searchTool) searchGoogle(ctx context.Context, req SearchRequest, log *slog.Logger) (string, error) {
	...
	if s.secretProvider != nil && s.cfg.GoogleCX != "" && (s.cfg.GoogleAPIKey == "" || s.cfg.UseGoogleOAuth) {
		result, err := s.searchGoogleWithOAuth(ctx, req.Query, log)
		if err == nil {
			return result, nil
		}
...
func (s *searchTool) searchGoogleWithOAuth(ctx context.Context, query string, log *slog.Logger) (string, error) {
