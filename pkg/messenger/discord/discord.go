...
func (m *Messenger) messageCreate(_ *discordgo.Session, event *discordgo.MessageCreate) {
	...
	if event.MessageReference != nil && event.MessageReference.MessageID != "" {
		metadata = map[string]any{messenger.QuotedMessageID: event.MessageReference.MessageID}
	}
...
// FormatClarification returns the request unchanged — Discord does not use
