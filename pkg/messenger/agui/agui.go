...
// CapabilitiesStance exposes the effective AI stance for the UI and community:
// which tools are available, which skip approval, and which are denied.
// When set, the server exposes GET /api/v1/capabilities so clients can show
// "what Genie can do" and approval policy without exposing secrets.
