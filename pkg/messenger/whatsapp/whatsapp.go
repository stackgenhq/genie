...
func New(cfg Config, opts ...messenger.Option) (*Messenger, error) {
	storePath := filepath.Join(osutils.GenieDir(), "whatsapp")
	if err := os.MkdirAll(storePath, 0o755); err != nil {
		return nil, fmt.Errorf("whatsapp: failed to create store dir %s: %w", storePath, err)
	}
...
func (m *Messenger) Connect(ctx context.Context) (http.Handler, error) {
	...
	if err := os.MkdirAll(m.storePath, 0o700); err != nil {
		return nil, fmt.Errorf("whatsapp: failed to create store dir %s: %w", m.storePath, err)
	}

	// Initialize SQLite-backed device store.
	dbPath := filepath.Join(m.storePath, "whatsmeow.db")
...
func (m *Messenger) ConnectionInfo() string {
	return fmt.Sprintf("Connected via WhatsApp Web protocol (store: %s) — scan QR code or message the linked number", m.storePath)
}
...
func (m *Messenger) handleMessage(ctx context.Context, evt *events.Message) {
	...
	mediaDir := filepath.Join(m.storePath, "media")
	...
	if quotedMsgID != "" {
		incoming.Metadata = map[string]any{
			messenger.QuotedMessageID: quotedMsgID,
		}
	}
...
// UpdateMessage is a no-op for WhatsApp — the adapter does not currently
