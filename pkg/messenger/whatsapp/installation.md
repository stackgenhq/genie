...
[messenger.whatsapp]
store_path = "~/.genie/whatsapp"
```

| Parameter | Description | Default |
|---|---|---|
| `store_path` | Directory for session/credential storage (SQLite database) | `~/.genie/whatsapp` |

The `store_path` is where Genie stores the encrypted session credentials. After the initial QR code pairing, this folder is all that's needed to reconnect automatically.
...
| Item | Description |
|---|---|
| `store_path` | Local directory for session storage (default: `~/.genie/whatsapp`) |
