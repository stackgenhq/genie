...
var _ = Describe("Config", func() {
	...
	Describe("PendingGraphLearnPath", func() {
		It("returns a path ending with graph_learn_pending filename", func() {
			path := graph.PendingGraphLearnPath("test-agent")
			Expect(filepath.Base(path)).To(Equal(graph.GraphLearnPendingFilename))
			// The sanitizer may replace hyphens with underscores.
		...
		It("uses default agent dir when name is empty", func() {
			path := graph.PendingGraphLearnPath("")
			Expect(filepath.Base(path)).To(Equal(graph.GraphLearnPendingFilename))
		})

		// Verify the path is writable (doesn't panic or create invalid names).
		It("returned path is valid for file operations", func() {
			path := graph.PendingGraphLearnPath("coverage-test")
			// Just verify the parent directory logic doesn't break — don't create files.
			dir := filepath.Dir(path)
			Expect(dir).NotTo(BeEmpty())
			// Clean up any created dirs if needed
			_ = os.RemoveAll(dir)
		})
	})
})
