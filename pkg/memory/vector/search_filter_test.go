...
var _ = Describe("Search with Filter", func() {
	var (
		ctx   context.Context
		store *vector.Store
	)
	...
	Context("filter-only mode (empty query with filters)", func() {
		...
		It("returns matching documents when query is empty and filter is provided", func() {
			results, err := store.Search(ctx, vector.SearchRequest{
				Query: "", Limit: 10,
		...
		It("returns all entities when filtering by type only", func() {
			results, err := store.Search(ctx, vector.SearchRequest{
				Query: "", Limit: 10,
				Filter: map[string]string{"__graph_type": "entity"},
			})
			Expect(err).NotTo(HaveOccurred())
		...
		It("returns relations when filtering for relation type", func() {
			results, err := store.Search(ctx, vector.SearchRequest{
				Query: "", Limit: 10,
				Filter: map[string]string{"__graph_type": "relation"},
			})
			Expect(err).NotTo(HaveOccurred())
			Expect(results).To(HaveLen(1))
	...
	Context("empty query guard", func() {
		It("returns error when both query and filter are empty", func() {
			results, err := store.Search(ctx, vector.SearchRequest{Query: "", Limit: 10})
		...
		It("returns error when query is empty and filter is an empty map", func() {
			results, err := store.Search(ctx, vector.SearchRequest{Query: "", Limit: 10, Filter: map[string]string{}})
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring("non-empty query or at least one filter"))
			Expect(results).To(BeNil())
		})
	...
	Context("vector search mode (non-empty query)", func() {
		BeforeEach(func() {
			err := store.Add(ctx, vector.AddRequest{Items: []vector.BatchItem{
				{ID: "v1", Text: "kubernetes pod restart", Metadata: map[string]string{"type": "k8s"}},
				{ID: "v2", Text: "aws s3 bucket policy", Metadata: map[string]string{"type": "aws"}},
			}})
			Expect(err).NotTo(HaveOccurred())
		})

		It("performs semantic search with non-empty query and no filter", func() {
			results, err := store.Search(ctx, vector.SearchRequest{Query: "kubernetes pod restart", Limit: 5})
			Expect(err).NotTo(HaveOccurred())
		...
		It("performs semantic search with non-empty query and filter", func() {
			results, err := store.Search(ctx, vector.SearchRequest{
				Query: "pod restart", Limit: 5,
				Filter: map[string]string{"type": "k8s"},
			})
			Expect(err).NotTo(HaveOccurred())
			Expect(results).NotTo(BeEmpty())
	...
	Context("Search delegates to unified Search", func() {
		It("Search with empty string returns error (guard propagates)", func() {
			_, err := store.Search(ctx, vector.SearchRequest{Query: "", Limit: 1})
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring("non-empty query or at least one filter"))
		})

		It("Search with non-empty string returns results normally", func() {
			err := store.Add(ctx, vector.AddRequest{Items: []vector.BatchItem{{ID: "s1", Text: "test doc"}}})
			Expect(err).NotTo(HaveOccurred())

			results, err := store.Search(ctx, vector.SearchRequest{Query: "test doc", Limit: 1})
			Expect(err).NotTo(HaveOccurred())
			Expect(results).To(HaveLen(1))
		})
	})
