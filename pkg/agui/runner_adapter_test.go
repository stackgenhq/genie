...
// TestRunnerRace is a standalone test designed to be run with -race flag.
// It specifically tests the blind spot #1 fix: rawChan channel ownership.
func TestRunnerRace(t *testing.T) {
	t.Parallel()

	chatFunc := func(ctx context.Context, msg string, ch chan<- interface{}) error {
		// Write many events rapidly — the race detector will catch
		// any concurrent read/write/close on the channel.
		for i := 0; i < 100; i++ {
			ch <- &event.Event{Response: &model.Response{
				Choices: []model.Choice{{Message: model.Message{Content: "race"}}},
			}}
		}
		return nil
	}
