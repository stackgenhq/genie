...
func newSchedulerWithDispatcher(cfg cron.Config, store cron.ICronStore, dispatcher cron.EventDispatcher) *cron.Scheduler {
	return cfg.NewScheduler(store, dispatcher)
}
...
var _ = Describe("Scheduler", func() {
	...
	Describe("Start", func() {
		It("should start even when Enabled config field is false", func() {
			...
			sched := newSchedulerWithDispatcher(cron.Config{}, fakeStore, dispatcher)
		...
		It("should upsert config-defined tasks on start", func(ctx context.Context) {
			...
			sched := newSchedulerWithDispatcher(cfg, fakeStore, dispatcher)
			sched.SetTickerIntervalForTest(50 * time.Millisecond)
