...
> **Startup Requirements:** Before executing any `kubectl` commands to inspect the cluster, you must first generate your kubeconfig by running `aws eks update-kubeconfig --region $AWS_REGION --name $EKS_CLUSTER_NAME`.
...
- **You MUST instruct the sub-agent** to run `aws eks update-kubeconfig` first if it needs to use K8s tools.
