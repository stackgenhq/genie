...
- [Build Instructions](#build-instructions)
  - [Lint, Format, and Test Must Be Green](#lint-format-and-test-must-be-green-mandatory)
...
### Lint, Format, and Test Must Be Green (MANDATORY)

**`make lint`, `make fmt` (or `make fmt/fix`), and `make test` MUST always be green.** No change may be committed or merged that breaks any of these.

#### Rules

- **Before committing**: Run `make lint`, `make fmt`, and `make test` (or `make fmt/fix` then `make fmt`). All must exit with success (zero exit code).
- **Agents**: After any code change, run these commands and fix any failures before considering the task complete.
- **PRs**: CI and reviewers expect lint, format, and tests to pass; do not open or merge PRs with red lint/fmt/test.

#### Commands

- `make lint` — runs golangci-lint; must report 0 issues.
- `make fmt` — checks that code is formatted with `go fmt`; if it fails, run `make fmt/fix` then re-run `make fmt`.
- `make fmt/fix` — applies `gofmt -w .` to fix formatting.
- `make test` — runs the unit test suite; all tests must pass.

#### Anti-Patterns to Avoid

- ❌ Committing or pushing when `make lint`, `make fmt`, or `make test` fails
- ❌ Leaving lint/format/test failures for "later" or for others to fix
- ❌ Disabling or skipping tests or linters to get a green build
...
**All constants, variables, types, functions, and methods MUST only be exported if they are used outside the package.**

#### Rules

- **Export Only When Needed**: Only export constants, variables, types, functions, and methods if they are used by other packages
- **Do Not Export Package-Internal Methods**: Do not export a method if it is only used within the same package; keep it unexported (lowercase first letter)
...
- **Don't Export**: Methods that are only used within the same package
...
Before exporting a constant, variable, type, function, or method:
...
- ❌ Exporting methods that are only used within the same package
...
- **Never pass logger as a parameter**: Do not add a `logger` or `log` argument to functions. Obtain the logger from context inside the function (e.g. `logger.GetLogger(ctx)` or `backend.Logger.FromContext(ctx)`). If a child function has no context, have it return an error with `fmt.Errorf` and let the parent log it if needed.
- **Context-Based Logging**: Always use `backend.Logger.FromContext(ctx)` or `logger.GetLogger(ctx)` instead of direct global logger calls when a context is available
...
- ❌ Passing logger (or log) as a function parameter — obtain from context or return errors for the parent to log
...
This repo does not require a Linear ticket ID in the PR title.
...
6. ✅ Verify constants, variables, types, functions, and methods are only exported if used outside the package
...
16. ✅ Verify all logging uses `backend.Logger.FromContext(ctx)` or `logger.GetLogger(ctx)` when context is available, and that logger is never passed as a function parameter
...
5. **Export Only When Necessary**: Unexport constants, variables, types, functions, and methods that are only used within the package
