...
### 5. Business Error Standards (MANDATORY)

**All business errors MUST use `problems.New` instead of `fmt.Errorf` to ensure consistent error handling and proper HTTP status codes.**

#### Rules

- **Business Errors**: All business validation errors, domain rule violations, and user-facing errors must use `problems.New`
- **HTTP Status Codes**: Business errors must include appropriate HTTP status codes (e.g., `http.StatusBadRequest`, `http.StatusNotFound`)
- **Error Codes**: Business errors must include error codes for programmatic error handling
- **Error Context**: Use the `extras` parameter to provide additional context (IDs, field names, values, etc.)
- **Technical Errors**: Technical errors that wrap other errors may still use `fmt.Errorf` to preserve error chains, but underlying business errors should use `problems.New`

#### Benefits of Using problems.New

- **Consistent Error Format**: All business errors follow the same structured format
- **HTTP Status Codes**: Proper status codes enable correct HTTP responses
- **Error Codes**: Error codes enable programmatic error handling in clients
- **Error Context**: Structured extras provide additional debugging information
- **Error Handling**: Problems can be properly handled by error handlers and converted to appropriate HTTP responses

#### Correct Pattern - Business Errors

**Correct - Use problems.New for business errors:**

```go
import (
    "net/http"
    "github.com/appcd-dev/go-lib/problems"
)

func (p *Parser) ParseSpec(spec string) (*SLO, error) {
    if spec == "" {
        return nil, problems.New("spec is required", http.StatusBadRequest, "INVALID_SPEC", nil)
    }
    
    objects, err := decodeSpec(spec)
    if err != nil {
        return nil, problems.New("failed to decode spec", http.StatusBadRequest, "OPENSLO_VALIDATION_ERROR", map[string]interface{}{
            "error": err.Error(),
        })
    }
    
    if len(objects) == 0 {
        return nil, problems.New("no SLO object found in spec", http.StatusBadRequest, "OPENSLO_VALIDATION_ERROR", nil)
    }
...
**Incorrect - Using fmt.Errorf for business errors:**
...
func (p *Parser) ParseSpec(spec string) (*SLO, error) {
    ...
    if len(objects) == 0 {
        return nil, fmt.Errorf("no SLO object found in spec")
    }
    
    return objects[0], nil
}

// ✅ Do this instead - use problems.New
func (p *Parser) ParseSpec(spec string) (*SLO, error) {
    if spec == "" {
        return nil, problems.New("spec is required", http.StatusBadRequest, "INVALID_SPEC", nil)
    }
...
#### When to Use problems.New

- **Business Validation Errors**: Validation failures, missing required fields, invalid values
- **Domain Rule Violations**: Business rule violations (e.g., "multiple SLO objects found", "invalid budget target")
- **User-Facing Errors**: Errors that should be returned to API clients with proper HTTP status codes
- **Structured Errors**: Errors that need error codes for programmatic handling

#### When to Use fmt.Errorf

- **Technical Errors**: Errors that wrap other errors to preserve error chains (but underlying business errors should use `problems.New`)
- **Internal Errors**: Errors that are not directly exposed to API clients
- **Error Propagation**: When propagating errors from lower layers that are already problems

#### Error Code Guidelines

- **Use Descriptive Codes**: Error codes should be descriptive and consistent (e.g., `OPENSLO_VALIDATION_ERROR`, `SLO_NOT_FOUND`, `INVALID_SPEC`)
- **Use Uppercase with Underscores**: Follow the pattern `CATEGORY_DESCRIPTION` (e.g., `OPENSLO_VALIDATION_ERROR`)
- **Be Specific**: Use specific error codes rather than generic ones when possible

#### HTTP Status Code Guidelines

- **400 Bad Request**: Validation errors, invalid input, malformed requests
- **404 Not Found**: Resource not found errors
- **409 Conflict**: Resource conflicts, duplicate entries
- **422 Unprocessable Entity**: Valid format but business rule violations
- **500 Internal Server Error**: Reserved for technical errors (typically not business errors)

#### Anti-Patterns to Avoid

- ❌ Using `fmt.Errorf` for business validation errors
- ❌ Missing HTTP status codes in business errors
- ❌ Missing error codes in business errors
- ❌ Not providing context in the `extras` parameter when helpful
- ❌ Using generic error codes instead of specific ones
- ❌ Using technical error status codes (500) for business errors

#### Examples

- **Correct pattern**: See [`pkg/domain/slo/spec_parser.go`](pkg/domain/slo/spec_parser.go) - business errors use `problems.New` with proper status codes and error codes
- **Correct pattern**: See [`pkg/domain/slo/extractor.go`](pkg/domain/slo/extractor.go) - validation errors use `problems.New`
- **Correct pattern**: See [`pkg/service/slo.go`](pkg/service/slo.go) - service layer uses `problems.New` for business errors
