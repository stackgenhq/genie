...
- **Agent Learning Loop** — After each completed task, a background `Learner` evaluates the task for novelty (1-10 score via LLM) and, if novel enough (≥7), distils a reusable skill in `agentskills.io` format. Skills are persisted to `~/.genie/<agent_name>/dynamic_skills/` and indexed in the vector store for semantic discovery via `memory_search` with `type: learned_skill` filter.
- **Sub-Agent Session Persistence** — Sub-agent conversations spawned via `create_agent` are now persisted to the database via an injected `session.Service`, enabling learning from sub-agent interaction history.
...
- **Orchestrator Persona Update** — Orchestrator persona now instructs the agent to search memory for available learned skills before starting work, ask the user for confirmation if a relevant skill is found, and mention the skill to the user.
