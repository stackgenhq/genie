...
func (c *connectCmd) command() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: fmt.Sprintf(`Connect to a running Genie AG-UI HTTP/SSE server and start an
interactive chat session. The server must be started with
"genie grant" before connecting.

Examples:
  # Default (localhost:%[1]d)
  genie connect

  # Custom URL
  genie connect --url http://genie.internal:%[1]d`, messenger.DefaultAGUIPort),
		RunE: c.run,
	}

	cmd.Flags().StringVar(&c.url, "url", osutils.Getenv("GENIE_AGUI_URL", fmt.Sprintf("http://localhost:%d", messenger.DefaultAGUIPort)), "URL of the Genie AG-UI server")
